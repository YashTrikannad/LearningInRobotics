...
class Ukf_base_class(QuaternionClass):
    ...
    def __init__(self):
        self.x_cap_k_1 = np.array([0.5, 0.5, 0.5, 0.5, 0.1, 0.1, 0.1])  # State Vector at k-1
        ...
        self.Q = 5e-8*np.eye(6)  # Noise Matrix for State Update
        self.R = 2e-5*np.eye(3)  # Noise Matrix for Measurement Update
        ...
        self.vk = np.array([])
        self.Kk = np.array([])
        self.g = np.array([[0], [0], [0], [9.8]])
        self.g_ = np.array([])
        self.state6d = np.zeros((6, 1))
...
class MeasurementModel(Ukf_base_class):
    ...
    def getMeasurements(self, zk, sensor="gyro"):
        if sensor == "gyro":
            ...
            self.g_ = self.quaternion_multiplication(self.quaternion_multiplication(self.Yi[:4, ], self.g),
                                                      self.quaternion_inverse(self.Yi[:4, ]))
    ...
    def kalman_update(self):
        ...
        self.x_cap_k[:4] = np.squeeze(self.quaternion_multiplication(self.x_cap_k[:4],
                                           np.squeeze(self.convert3dto4d(update[:3]))))
        self.x_cap_k[4:] = self.x_cap_k[4:] + np.squeeze(update[3:])
...
class Model(MeasurementModel, process):
    ...
    def getState(self):
        self.state6d[:3] = self.convert4dto3d(np.expand_dims(self.x_cap_k[:4], axis=1))
        self.state6d[3:] = np.expand_dims(self.x_cap_k[4:], axis=1)
        return self.state6d
